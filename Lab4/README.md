# Lab 4 Data transformation in bash

## Technologies
```
To transform data we can use tools like head,cat,paste,awk,join,echo,date etc. (everything avalaible on bash shell).
I transform two input files to three output .txt files which are similiar to tables in databases.
```
## Get million songs dataset
```
bash get_dataset.sh
```
